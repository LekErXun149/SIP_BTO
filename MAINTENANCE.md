...
Technical documentation for KeyQuest — how the site is built, where to change things,
and the decisions that shouldn't be undone by accident.

For what the project is and why it exists, see [README.md](README.md).
...
**Figures come from HDB, not from memory or a search result.** Housing policy moved
four times during this project: the income ceiling rose mid-build, HDB restructured
their entire website and broke every source link, the Married Child Priority Scheme was
quietly replaced, and the EHG band tables turned out not to follow a formula.

Every number on the site is traceable to an official page listed in `BTO_DATA.sources`.
Keep it that way. Property blogs and search results are frequently months out of date.

---

## The nine pages

`index` `calculator` `journey` `ballot` `guide` `options` `quiz` `checklist`, plus `404`.
See [README.md](README.md) for what each one covers.

All nine share `css/style.css`, `data/bto-data.js`, `js/nav.js` and `js/progress.js`.
Change any of those and every page changes — which is the point, but also means bumping
the cache version and re-running the tests.
...
Five suites, each loading a real page in a simulated browser and driving it the way a
user would. They need `jsdom`:

```bash
npm install jsdom
node test_calc.js     # calculator inputs: sliders paired with number boxes
node test_borrow.js   # loan amount, downpayment, zero-loan cases
node test_ehg.js      # every EHG band against HDB's published PDFs
node test_quiz.js     # quiz scoring, per-option feedback, saving
node test_mobile.js   # layout traps that only show on a phone
```

Run the relevant suite after changing anything in `js/` or `data/`. They exist because
these bugs were subtle — the numbers looked plausible and were quietly wrong.
